...
class PersonnelPage:
    ...
    def choose_photo(self):
        selected = filedialog.askopenfilename(filetypes=[("Images", "*.png *.jpg *.jpeg *.bmp")])
        if selected:
            self.photo_path = selected
            self.photo_status.configure(text=os.path.basename(selected))

    def _init_camera(self, cv2):
        """Tente d'ouvrir la caméra principale via les principaux backends d'OpenCV."""
    ...
    def open_photo_camera(self):
        try:
            ...
            messagebox.showerror("Caméra indisponible", "OpenCV n'est pas installé.")
            return
        ...
        def update_preview():
            ...
            if ok and frame is not None:
                ...
                if Image and ImageTk:
                    ...
                    pil_image = Image.fromarray(rgb_frame).resize((400, 270))
    ...
    def open_face_scan(self):
        try:
            ...
            messagebox.showerror("FaceId indisponible", "OpenCV, Pillow ou face_utils est manquant.")
            return
        ...
        preview = ctk.CTkLabel(window, text="Initialisation de la caméra...", width=420, height=290)
        preview.pack(padx=12, pady=12)

        status = ctk.CTkLabel(window, text="Scan en cours... (60 secondes maximum)",
        ...
        state = {"running": True, "started": datetime.now().timestamp()}

        if camera is None:
            preview.configure(text="Impossible d'accéder à la caméra")
            status.configure(text="Vérifiez qu'aucune autre application n'utilise la caméra", text_color=DANGER_COLOR)
        ...
        def fail():
            ...
            if camera and camera.isOpened():
                camera.release()
            status.configure(text="Aucun visage détecté après 60 secondes.", text_color=DANGER_COLOR)
        ...
        def update():
            ...
            if datetime.now().timestamp() - state["started"] >= 60:
                fail()
                return
            ...
            if ok and frame is not None:
                ...
                # Dessiner un cadre visuel autour des visages détectés
                ...
                if len(faces) > 0:
                    ...
                    if face_id is not None:
                        ...
                        status.configure(text="Visage enregistré avec succès.", text_color=SUCCESS_COLOR)
                        state["running"] = False
                        camera.release()
                        window.after(900, close)
                        return

                # Inverser et afficher le flux d'images sur l'interface
                rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                pil_image = Image.fromarray(rgb_frame).resize((420, 290))
                photo = ImageTk.PhotoImage(pil_image)
                preview.configure(image=photo, text="")
                preview.image = photo
        ...
        def begin_scan():
            ...
            status.configure(text="Scan en cours... (60 secondes maximum)", text_color=TEXT_MUTED)
            update()
    ...
    def save_person(self):
        ...
        try:
            # Conversion du descripteur NumPy ndarray vers liste standard Python pour JSON/DB
